...
Deno.serve(async (req) => {
  try {
    ...
    await base44.data.set(dataKey, { private_key, key_type, created_at: new Date().toISOString() });

    return Response.json({ success: true, wallet_id, message: 'Private key stored' });
  } catch (error) {
