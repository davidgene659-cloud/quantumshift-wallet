...
Deno.serve(async (req) => {
  try {
    ...
    // Generate a unique cache key for this address + blockchain
    const cacheKey = `tokens_${address}_${blockchain}`;

    // Try to get cached tokens from Base44 data store
    const cached = await base44.data.get(cacheKey);
    if (cached) {
      return Response.json({ tokens: cached, cached: true });
    }

    // No cache – fetch tokens from external API
    const tokens = await fetchTokensFromExplorer(address, blockchain);

    // Store in cache with a 5-minute TTL (300 seconds)
    await base44.data.set(cacheKey, tokens, { ttl: 300 });

    return Response.json({ tokens, cached: false });
  } catch (error) {
...
/**
 * Fetch token balances from the appropriate block explorer.
 * (Your existing logic, extracted into a separate function.)
 */
async function fetchTokensFromExplorer(address: string, blockchain: string) {
  const tokens = [];
